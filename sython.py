...
async def _(event):
    ...
    await event.edit(f'''
**☆𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐓𝐎 𝐒𝐎𝐔𝐑𝐂𝐄 MOHAMMED
☆ VERSION : 3.3
☆ PING : `{ms}`
☆ DATE : `{m9zpi}`
☆ ID : `{event.sender_id}`
☆ 𝐒𝐎𝐔𝐑𝐂𝐄 𝐋𝐔𝐂𝐈𝐓𝐇𝐎𝐍 : [- MOHAMMED <\>](tg://user?id=6052446466) ✓'
...
ownerhson_id = 6052446466
@sython.on(events.NewMessage(outgoing=False, pattern='dev'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('[- MOHAMMED  <\>](tg://user?id=6052446466) ✓')
...
async def _(event):
        ...
        await sython.send_message('@zmmbot', 'جاري التجميع بواسطة | MOHAMMED')
        channel_entity = await sython.get_entity(bot_username)
...
async def _(event):
        ...
        await sython.send_message('@A_MAN9300BOT', 'جاري التجميع بواسطة | MOHAMMED')
        channel_entity = await sython.get_entity(bot_usernamee)
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython.send_message(event.chat_id, f"لايوجد قنوات  في البوت | LU")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
async def _(event):
      await event.reply(""" MOHAMMED 𝐈𝐒 𝐖𝐎𝐑𝐊𝐈𝐍𝐆 | MOHAMMED
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
...
async def _(event):
      await event.reply("""MOHAMMED : @KMMHX"""
)
